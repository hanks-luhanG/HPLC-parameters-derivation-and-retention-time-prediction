...
# Script:  Neue_Kuss_Model_Fitting.R 
...
#              based on the Neue Kuss model for RP-HPLC data.
...
# [1.1 training ODE]
ode_func_nk <- function(t, L, parms) {
  t_prime <- t - parms$td
  if (t_prime <= 0) phi <- parms$phi0
  else if (t_prime <= parms$t_grad_end) phi <- parms$phi0 + parms$B * t_prime
  else phi <- parms$phi1
  exponent <- -(parms$b_nk * phi) / (1 + parms$c_nk * phi + 1e-9)
  term_exp <- exp(pmax(pmin(exponent, 100), -100))
  term_ln <- (1 + parms$c_nk * phi)^(2)
  k_current <- parms$kw * term_ln * term_exp
  dLdt <- parms$u_linear / (1 + k_current)
  return(list(c(L = dLdt)))
}

# [1.2 prediction ODE]
ode_func_prediction_nk <- function(t, L, parms) {
  ...
  exponent <- -(parms$b_nk * phi_eff) / (1 + parms$c_nk * phi_eff + 1e-9)
  term_exp <- exp(pmax(pmin(exponent, 100), -100))
  term_ln <- (1 + parms$c_nk * phi_eff)^(2)
  k_current <- parms$kw * term_ln * term_exp
  dLdt <- parms$u_linear / (1 + k_current)
  return(list(c(L = dLdt)))
}
...
stable_integrate <- function(par, model_type, single_row_data){
  lnkw <- par[1]; b_nk <- par[2]; c_nk   <- if (model_type == "three_par") par[3] else 0
  parms <- list(kw = exp(lnkw), b_nk = b_nk, c_nk = c_nk,
  ...
  ode_res <- tryCatch({
    deSolve::ode(y = c(L = 0), times = c(0, t_end), func = ode_func_nk, 
...
objective <- function(par, model_type, fit_data) {
  lnkw <- par[1]; b_nk <- par[2]; c_nk   <- if (model_type == "three_par") par[3] else 0
  if (lnkw < -15 || lnkw > 15 || b_nk < 0.01 || b_nk > 30 ||
      (model_type == "three_par" && (c_nk < -30 || c_nk > 30))) return(1e9)
...
predict_t_R_robust_new <- function(lnkw, b_nk, c_nk, target_L, Q_flow, td_delay, phi0_start, phi_values_end, stage_boundaries) {
  A_cm2 <- pi * (4.6 / (2 * 10))^2
  ...
  parms_pred <- list(kw = exp(lnkw), b_nk = b_nk, c_nk = c_nk,
                     u_linear = u_linear, td = td_delay, B_values = B_values,
                     phi_start_of_stage = phi_start_of_stage, 
                     stage_boundaries = stage_boundaries, 
                     stage_start_times = stage_start_times)
  
  obj_func <- function(t_R) {
    if (t_R <= 0) return(target_L)
    res <- deSolve::ode(y = c(L = 0), times = c(0, t_R), func = ode_func_prediction_nk, parms = parms_pred)
...
raw_data_input <- read_excel(path_to_file, sheet = "acetonitrile-Hypersil GOLD AQ")

# function starting
cl <- makeCluster(detectCores() - 1)
clusterEvalQ(cl, library(deSolve))
clusterExport(cl, c("ode_func_nk", "stable_integrate", "objective", "objective_stabilized", "ode_func_prediction_nk"))
...
for (i in 1:nrow(raw_data_input)) {
  ...
  # [step 2: Neue-Kuss model optimization（3-param）]
  results_nk <- parLapply(cl, 1:10, function(idx, f_data, l_ref, s_ref) {
    set.seed(123 + idx)
  ...
  best_nk <- results_nk[[which.min(sapply(results_nk, `[[`, "value"))]]

  # [step 3: prediction]
  pred_tR <- predict_t_R_robust_new(
    lnkw = best_nk$par[1], b_nk = best_nk$par[2], c_nk = best_nk$par[3],
  ...
  final_output_list[[i]] <- data.frame(
    ...
    lnkw = best_nk$par[1], b_nk = best_nk$par[2], c_nk = best_nk$par[3],
    SSE = best_nk$value, Predicted_tR = pred_tR
  )
}

stopCluster(cl)
write_xlsx(bind_rows(final_output_list), "./Neue_Kuss_Model_results.xlsx")
cat("\n✅ Fully calculated！")
